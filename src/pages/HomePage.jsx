...
import { setGames, setCurrentIndex } from '../redux/gameSlice';
...
const HomePage = () => {
  ...
  const { games, currentIndex } = useSelector((state) => state.games);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      dispatch(setCurrentIndex((prevIndex) => (prevIndex + 1) % games.length));
    }, 5000);
